...
| `##[CLOCK] {"black_remaining_ms":<ms>,"white_remaining_ms":<ms>,"side_to_move":"sente"\|"gote","ply":<n>}` `*` | **Workers 観戦者専用**。盤面を進めた各 `<token>,T<sec>` の直後に、サーバー時計の本体残時間 (ms) と次手番を送る。`ply` は直前の指し手と同じ値。viewer はローカル countdown の anchor を毎手この値で再同期し、`T<sec>` の秒丸めや通信遅延による累積誤差を防ぐ。既存 client は未知の `##[...]` 行として無視できる。対局者および TCP frontend には送らない | Workers `game_room.rs::GameRoom::dispatch_broadcasts` / `spectator_snapshot.rs::build_spectator_clock_update` |
