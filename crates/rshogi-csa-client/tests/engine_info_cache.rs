...
#[test]
fn isready_barrier_discards_stale_info_before_next_go() {
    let path = write_mock_engine(
        r#"#!/usr/bin/env bash
ready_count=0
go_count=0
while IFS= read -r line; do
    case "$line" in
        usi)
            echo "id name stale-info-barrier"
            echo "usiok"
            ;;
        isready)
            ready_count=$((ready_count + 1))
            if [ "$ready_count" -eq 3 ]; then
                echo "info depth 99 score cp -32001 nodes 0 pv 1c1d"
            fi
            echo "readyok"
            ;;
        position*)
