...
- 実測スケーリング（RTX 5090 native / TRT fp16 / 30m resnet10 / batch1024 / Ryzen 9 9950X3D 16C32T、
  各プロセス `--threads=16÷N`）: 1→**42.8k** / 2→85.7k(2.0x) / 3→96.6k / 4→119k / 5→128k /
  6→**143k(3.34x)** / 8→144k。**~6 プロセスでプラトー（約 3.3x）**、8 本は 6 本と同値＝それ以上は無駄。
- **プラトーの律速は VRAM ではなく CPU**（特徴量構築 producer が物理コアを使い切ると頭打ち。GPU は
  まだ余力があるが供給が追いつかない）。「VRAM の限り N 本」ではなく、**物理コア数で決まる数本
  （実測 ~6）が最適**。各プロセスは `--threads ≈ 物理コア数÷N` にしてオーバーサブスクライブを避ける。
- 入力分割の例: raw_4B を先頭/後半で分ける、`--input` を別 shard にする、`--limit` とオフセット別
  スライスにする、等。**各プロセスの `--output-dir` は必ず分ける**（同一だと `.done`
  マーカーで片方が全スキップする）。
