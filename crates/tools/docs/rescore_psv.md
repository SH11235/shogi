...
### 5. Windows ネイティブ環境の補足（RTX 5090 / Blackwell で動作確認済み）

Windows で使う場合は、上記 1〜3 を **Windows 版・同一バージョン**で揃える:

- **ONNX Runtime**: `onnxruntime-win-x64-gpu-1.24.2.zip`（**CUDA 12 ビルド**を使う。`onnxruntime-win-x64-gpu_cuda13-*.zip` ではない）
- **cuDNN 9**: `cudnn-windows-x86_64-9.8.0.87_cuda12-archive.zip`
- **TensorRT 10.11**: `TensorRT-10.11.0.33.Windows.win10.cuda-12.9.zip`
...
### スループット最適化: 同一 GPU で複数プロセス並走

軽量モデル（resnet10 等）＋高速 GPU では、**1 プロセスは前処理（read+build）供給律速**に
なりやすく、GPU が遊ぶ。このとき **入力を分割して `rescore_psv` を複数プロセス並走**させると、
各プロセスが独立の CUDA ストリーム（＋独立の前処理スレッド群）を持つため、GPU の空きを
埋めて実効スループットがほぼ線形にスケールする。**コード変更不要**。

- 実測スケーリング例（参考値・環境依存。resnet10 57ch / TRT fp16 / batch1024 / 16C32T CPU、
  各プロセス `--threads = 物理コア数 ÷ N`）: 1→**1.0x** / 2→2.0x / 3→2.3x / 4→2.8x / 5→3.0x /
  6→**3.34x** / 8→3.36x。**~6 プロセスでプラトー**、8 本は 6 本と同値＝それ以上は無駄。実効値は
  CPU コア数・モデル・同時負荷で変わる。
- **プラトーの律速は VRAM ではなく CPU**（特徴量構築 producer が物理コアを使い切ると頭打ち。GPU は
  まだ余力があるが供給が追いつかない）。「VRAM の限り N 本」ではなく、**物理コア数で決まる数本
  （この例では ~6）が最適**。各プロセスは `--threads ≈ 物理コア数÷N` にしてオーバーサブスクライブを避ける。
- 入力分割の例: 大きな入力ファイルを先頭/後半で分ける、`--input` を別 shard にする、`--limit` と
  オフセット別スライスにする、等。**各プロセスの `--output-dir` は必ず分ける**（同一だと `.done`
  マーカーで片方が全スキップする）。
- TRT engine cache（`--onnx-tensorrt-cache`）は共有可（同一 GPU/TRT なら再利用でビルド 1 回）。
- `nvidia-smi` の util と合算スループットを見て調整。プラトー後の真の伸びしろは producer(read+build)高速化。

> 単一プロセス内での GPU 多重化（2 stream in-flight）は `ort` の `run_binding` が `&mut self`
> で直列化される制約があり大改修が要る。まずは本手法（複数プロセス）で GPU 余力を回収するのが
> 手軽で確実。
