...
Pass 2 では複数チャンクを並列処理しますが、バッチは Pass 1 後の各チャンクの実ファイルサイズと利用可能メモリの 70% に基づいて自動編成されます。Linux コンテナでは cgroup の残量も上限に反映します。メモリ不足の場合は処理開始前にエラーで停止します（`--force` で強制続行可能）。
