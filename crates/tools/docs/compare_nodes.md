...
| `--depth <N>` | 10 | 探索深度 |
| `--nodes-a <N>` | 未指定 | エンジンAの追加ノード上限（1以上）。指定時は `go depth N nodes M` を送信 |
| `--nodes-b <N>` | 未指定 | エンジンBの追加ノード上限（1以上）。指定時は `go depth N nodes M` を送信 |
...
| `results.jsonl` | 局面ごとの深度別ノード数、評価値、PV、最善手、差分、処理時間 |
| `summary.txt` | 設定ヘッダ、深度別集計、最善手一致率、乖離分布、乖離が大きい局面 |
