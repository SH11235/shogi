...
| `search_only_ab` | search-only A/B ベンチマーク。起動・ロード時間を除外して cycles/node, instructions/node を正確計測。Linux は `perf stat --control`、Windows は ETW NT Kernel Logger の PMC counting（要管理者権限、`--pmc-sources` で source 指定、Hyper-V/VBS 共存可）。CLI / JSON レポートは両 OS でスキーマ互換（Windows は `--cpus` shard 並列のみ未対応） |
