...
pub fn read_compressed_tensor_i16_all<R: Read>(reader: &mut R) -> io::Result<Vec<i16>> {
    ...
    // 不正ファイルの巨大 alloc を防ぐ sanity 上限。HalfKa-E4 は base 特徴を NB 倍に
    // 拡張するため FT block が大きく (2x2×1024=600MB 生 / ~300MB 圧縮、KPE9 系はさらに大)、
    // 旧 256MB では正当な E4 net を弾く。size field は u32 なので上限は 4GiB 未満。
