...
impl PositionHasher {
    ...
    pub fn new() -> Self {
        ...
        for square in 0..81 {
            for piece in 0..32 {
                zobrist_pieces[square][piece] = Self::next_random(&mut rng_state);
            }
        }
        ...
        for i in 0..14 {
            zobrist_hand[i] = Self::next_random(&mut rng_state);
        }
    ...
    fn get_piece_index(&self, piece_type: char, promoted: bool) -> Result<usize> {
        ...
        let player_offset = if piece_type.is_ascii_uppercase() { 0 } else { 8 };
