...
describe("applyMove / revertMove", () => {
    it("通常の移動を適用・巻き戻し", () => {
        ...
        expect(applied.board["66"]).toMatchObject({
            type: "pawn",
            owner: "black",
            promoted: false,
        });
        ...
        expect(reverted.board["76"]).toMatchObject({
            type: "pawn",
            owner: "black",
            promoted: false,
        });
    ...
    it("成りを適用・巻き戻し", () => {
        ...
        const move: Move = {
            ...
            from: sq(2, 2),
            to: sq(8, 8),
            piece: makePiece("bishop", "white"),
            promote: true,
            captured: null,
        };

        const applied = applyMove(board, hands, "white", move);
        expect(applied.board["88"]).toMatchObject({
            type: "bishop",
            owner: "white",
            promoted: true,
        });
        ...
        const reverted = revertMove(applied.board, applied.hands, applied.nextTurn, move);
        expect(reverted.board["22"]).toMatchObject({
            type: "bishop",
            owner: "white",
            promoted: false,
        });
    ...
    it("駒の取得を適用・巻き戻し", () => {
        ...
        const move: Move = {
            ...
            piece: makePiece("bishop", "white"),
            promote: false,
            captured: makePiece("rook", "black"),
        };

        const applied = applyMove(board, hands, "white", move);
        expect(applied.hands.white.飛).toBe(1);
        expect(applied.board["88"]).toMatchObject({ type: "bishop", owner: "white" });

        const reverted = revertMove(applied.board, applied.hands, applied.nextTurn, move);
        expect(reverted.hands.white.飛).toBe(0);
        expect(reverted.board["88"]).toMatchObject({ type: "rook", owner: "black" });
    });
    ...
    it("打ち手を適用・巻き戻し", () => {
        ...
        expect(applied.board["55"]).toMatchObject({
            type: "pawn",
            owner: "black",
            promoted: false,
        });
...
describe("generateMoves", () => {
    ...
    it("角はスライド移動（8方向に複数マス）", () => {
        let board: Board = { ...nullBoard };
        board = setPiece(board, sq(5, 5), makePiece("bishop", "black"));
    ...
    it("玉は8方向に1マス（後手の玉）", () => {
        let board: Board = { ...nullBoard };
        board = setPiece(board, sq(5, 5), makePiece("gyoku", "white"));
...
describe("hasNifuViolation", () => {
    ...
    it("同じ筋に相手の歩がある場合はfalseを返す", () => {
        let board = nullBoard;
        board = setPiece(board, sq(3, 1), makePiece("pawn", "white"));
        expect(hasNifuViolation(board, 1, "black")).toBe(false);
    });

    it("同じ筋に成った歩（と金）がある場合はfalseを返す", () => {
        let board = nullBoard;
        board = setPiece(board, sq(3, 1), makePiece("pawn", "black", true));
        expect(hasNifuViolation(board, 1, "black")).toBe(false);
    });
});
...
describe("isDeadPiece", () => {
    it("歩が先手の最奥段（1段目）に置かれる場合はtrue", () => {
        expect(isDeadPiece(makePiece("pawn", "black"), 1)).toBe(true);
    });

    it("歩が後手の最奥段（9段目）に置かれる場合はtrue", () => {
        expect(isDeadPiece(makePiece("pawn", "white"), 9)).toBe(true);
    });

    it("香が先手の最奥段（1段目）に置かれる場合はtrue", () => {
        expect(isDeadPiece(makePiece("lance", "black"), 1)).toBe(true);
    });

    it("桂が先手の最奥2段（1,2段目）に置かれる場合はtrue", () => {
        expect(isDeadPiece(makePiece("knight", "black"), 1)).toBe(true);
        expect(isDeadPiece(makePiece("knight", "black"), 2)).toBe(true);
        expect(isDeadPiece(makePiece("knight", "black"), 3)).toBe(false);
    });

    it("桂が後手の最奥2段（8,9段目）に置かれる場合はtrue", () => {
        expect(isDeadPiece(makePiece("knight", "white"), 8)).toBe(true);
        expect(isDeadPiece(makePiece("knight", "white"), 9)).toBe(true);
        expect(isDeadPiece(makePiece("knight", "white"), 7)).toBe(false);
    });

    it("その他の駒は常にfalse", () => {
        expect(isDeadPiece(makePiece("gold", "black"), 1)).toBe(false);
        expect(isDeadPiece(makePiece("silver", "black"), 1)).toBe(false);
        expect(isDeadPiece(makePiece("bishop", "black"), 1)).toBe(false);
        expect(isDeadPiece(makePiece("rook", "black"), 1)).toBe(false);
        expect(isDeadPiece(makePiece("king", "black"), 1)).toBe(false);
    });
});
...
describe("applyMove - 特殊ルール", () => {
    ...
    it("打ち歩詰めの場合はエラーを投げる", () => {
        ...
        let board = nullBoard;
        board = setPiece(board, sq(1, 1), makePiece("gyoku", "white"));
        board = setPiece(board, sq(1, 2), makePiece("gold", "black"));
        board = setPiece(board, sq(2, 2), makePiece("silver", "black"));
        board = setPiece(board, sq(3, 1), makePiece("gold", "black"));
...
describe("isUchifuzume", () => {
    it("歩を打って詰みになる場合はtrueを返す", () => {
        ...
        board = setPiece(board, sq(1, 1), makePiece("gyoku", "white"));
        board = setPiece(board, sq(1, 2), makePiece("gold", "black")); // 1二
        board = setPiece(board, sq(2, 2), makePiece("silver", "black")); // 2二
        board = setPiece(board, sq(3, 1), makePiece("gold", "black")); // 3一
        board = setPiece(board, sq(1, 3), makePiece("silver", "black")); // 1三
        // この状態で2一に歩を打つと詰み
